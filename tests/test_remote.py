...
class TestRemote:
    ...
    def test_validate_response(self):
        ...
        response = Mock(**{'status_code': 200, 'raise_for_status.return_value': None})
    ...
    def test_request(self):
        ...
        with patch('passthesalt.remote.requests.request') as mock_request:
            resp = Mock(
                **{'status_code': 200, 'json.return_value': {'message': 'result'}}
            )
    ...
    def test_get(self):
        remote = Remote(location='https://api.store.com').with_auth(
            ('name', 'password')
        )

        with patch('passthesalt.remote.Remote.request') as mock_request:
            mock_request.return_value = (
                'eyJtb2RpZmllZCI6ICIyMDE4LTEyLTI1IDAwOjAwOjAwIn0='
            )
    ...
    def test_put(self):
        ...
        with patch('passthesalt.remote.Remote.request') as mock_request:
            pts = PassTheSalt(modified=datetime.datetime(year=2018, month=12, day=25))
            pts.version = (
                '3.0.0'  # the expected data below assumes this hardcoded version...
            )
            expected = (
...
class TestStow:
    ...
    def test_renew(self):
        ...
        with patch('passthesalt.remote.requests.request') as mock_request:
            mock_request.return_value = Mock(
                **{'status_code': 200, 'json.return_value': {'token': 'thetoken'}}
            )
    ...
    def test_put(self):
        ...
        with patch('passthesalt.remote.Stow.request') as mock_request:
            mock_request.return_value = Mock(
                **{'status_code': 200, 'json.return_value': {'message': 'success'}}
            )
